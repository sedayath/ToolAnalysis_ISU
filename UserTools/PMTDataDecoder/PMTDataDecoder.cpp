...
double PMTDataDecoder::firstNonZeroClockTimeInMs = 0.0;  // Initialization
...
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
   // Convert ClockCount to nanoseconds
    uint64_t ClockTimeInNs = ClockCount * 8;

    // Convert nanoseconds to milliseconds
    double ClockTimeInMs = static_cast<double>(ClockTimeInNs) / 1e9;

    // Static variable to store the first non-zero clock time in milliseconds
    static bool firstNonZeroClockFound = false;
//    static double firstNonZeroClockTimeInMs = 0;

    // Check if this is the first non-zero entry, and set it as the baseline
    if (!firstNonZeroClockFound && ClockTimeInMs > 0) {
        firstNonZeroClockTimeInMs = ClockTimeInMs;  // Save the first non-zero clock time
        firstNonZeroClockFound = true;              // Mark that we have found the first valid clock time
    }

    // Calculate the time difference from the first non-zero clock time
    double timeDifferenceInMs = ClockTimeInMs - firstNonZeroClockTimeInMs;

    std::vector<int> wave_key{CardID, ChannelID};
    std::vector<uint16_t> Waveform;
   // std::cout << "PMTDataDecoder Tool: Parsed Clock time (ms) for header is " << ClockTimeInNs << std::endl;
//std::cout << "PMTDataDecoder Tool: Parsed time difference (ms) from first non-zero entry is " << timeDifferenceInMs << std::endl;
    // Update the TriggerTimeBank and WaveBank with new entries
    Log("PMTDataDecoder Tool: Parsed Clock time (ms) for header is " + to_string(ClockTimeInMs), v_debug, verbosity);
    Log("PMTDataDecoder Tool: Parsed time difference (ms) from first non-zero entry is " + to_string(timeDifferenceInMs), v_debug, verbosity);
...
double Besttime = static_cast<double>(FinishedWaveTrigTime) / 1e9;
   if(CardID==1012){
 // std::cout<<"Finished wave trigger time in sec "<<Besttime<<std::endl;
   std::cout<<"Finished wave trigger time "<<FinishedWaveTrigTime<<std::endl;};
